...
- **一次升級的各條通道是獨立的，而那條「送不到任何人」的絕不能當成閘門。**
  ...
  **第一個** 敘述且沒有包起來，所以一次 publish 失敗會把其他三個一起帶走 —— 包括那個 settle，
  ...
  2026-08-02 提供。但要注意那個修法**沒有**解決的部分：email 訂閱在收件人點擊確認連結之前都是
  `PendingConfirmation`，而 `ensure_topic` 之所以把 pending 單獨報出來，正是因為「已訂閱但未
  確認」只是把同一種沉默往後挪了一步。所以下面的排序仍然是必要的，不是被取代了：一條通道的
  聽眾可以在沒有任何程式碼變動的情況下重新變成零。現在每一種通知都各自被包住並記錄，順序依「失去它的
