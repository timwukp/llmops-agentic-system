...
|---|---|---|
| Unit tests (contracts, cost model, driver loop, Lambdas, state machine document) | **817/817 passed** | `.venv/bin/python -m pytest tests/ -q --ignore=tests/golden` |
...
| Negative controls — every guard broken in turn, confirmed to fail | **124/124 negative controls** | `.venv/bin/python tests/negative_controls/monitor_dispatch.py` |
...
| Check | Result | How to reproduce |
|---|---|---|
| Unit tests (all suites, incl. `test_cost_model.py` + `test_finops.py`) | **817 passed** | `.venv/bin/python -m pytest tests/ -q` |
...
reverted one at a time and the test confirmed to fail — **124/124 negative controls**, 108
mutations asserting 124 (guard, mutation) pairs, one printed PASS line each. A test that
