...
|---|---|---|
| Unit tests (contracts, cost model, driver loop, Lambdas, state machine document) | **594/594 passed** | `.venv/bin/python -m pytest tests/ -q --ignore=tests/golden` |
...
| Check | Result | How to reproduce |
|---|---|---|
| Unit tests (all suites, incl. `test_cost_model.py` + `test_finops.py`) | **594 passed** | `.venv/bin/python -m pytest tests/ -q` |
