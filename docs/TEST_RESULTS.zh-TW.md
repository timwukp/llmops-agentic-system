...
| 檢查 | 結果 | 復現方式 |
|---|---|---|
| 單元測試（契約、成本模型、driver 迴圈、Lambda、狀態機文檔） | **1232/1232 通過** | `.venv/bin/python -m pytest tests/ -q --ignore=tests/golden` |
| Shell 測試套件 —— N 路 capacity race guard（`tests/test_capacity_race_guard.sh`） | **10/10 斷言** | `bash tests/test_capacity_race_guard.sh` |
| 反向控制 —— 逐一破壞每道 guard，確認它會失敗 | **253/253 反向控制** | `.venv/bin/python tests/negative_controls/monitor_dispatch.py` |
...
| 檢查 | 結果 | 重現方式 |
|---|---|---|
| 單元測試(全部套件,含 `test_cost_model.py` + `test_finops.py`) | **1232 passed** | `.venv/bin/python -m pytest tests/ -q` |
...
失敗 —— **253/253 反向控制**,217 個 mutation 斷言 253 組（guard, mutation）配對,runner 各印
