...
| 檢查 | 結果 | 復現方式 |
|---|---|---|
| 單元測試（契約、成本模型、driver 迴圈、Lambda、狀態機文檔） | **1620/1620 通過** | `.venv/bin/python -m pytest tests/ -q --ignore=tests/golden` |
...
| 檢查 | 結果 | 重現方式 |
|---|---|---|
| 單元測試(全部套件,含 `test_cost_model.py` + `test_finops.py`) | **1620 passed** | `.venv/bin/python -m pytest tests/ -q` |
