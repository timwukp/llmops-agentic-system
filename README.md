...
<!-- diagrams generated by docs/gen_architecture_svg.py; layout-verified by tests/check_svg_geometry.py -->
High-level: `docs/architecture-high-level.svg` · Low-level: `docs/architecture-low-level.svg` *(generated in Phase 6)*

```
TRIGGERS                      SPINE                             WORKER HARNESSES
...
Admin API (Cognito)   ─┤  │ Lambda        │  │ Functions    │  llmops_finetune    (llm-fine-tuning · llm-distillation ·
HMAC webhook          ─┘  └───────────────┘  │ Standard     │                      llm-cost-optimization)
        ...
        (InvokeHarness stream,               └──────────────┘  llmops_monitor     (llm-observability · llm-cost-optimization ·
         inline-fn toolUse ⇄ toolResult)            ▲                              llm-agent-orchestration)
   EventBridge: SageMaker job state ─▶ resume ──────┘
